...
public class FractalPanel extends JPanel {
	...
	public void paintComponent( Graphics g ) {
		 ...
		 */
		for(Line l: lines){
			drawKochLine(g2, level, (KochLine) l);    //drawing each Kochline 
		}
	...
	/**
	 * 
	 * drawKochLine draws kochlines recursively with an indicated number of level
	 * 
	 * @param g2    a Graphics2D object
	 * @param level     the depth of recursion when a kochline will be divided in other kochlines 
	 * @param line    the kochline to be drawn
	 */
	public void drawKochLine( Graphics2D g2, int level, KochLine line ) {
		...
		//reproducing the algorithm to draw a kochline, line by line
		line.computeFractal();    
		if(level == 0){
